...
class E(pygame.sprite.Sprite):
    ...
    def update(self):
        self.rect.y += int(self.base_speed * enemy_speed_boost)
        if self.rect.top > h:
            self.rect.y = -80
...
while run:
    ...
    for c in cs:
        if p.rect.colliderect(c.rect):
            ...
            c.value = random.choice([1, 2, 5])
            if coins // coin_goal > (coins - c.value) // coin_goal:
                enemy_speed_boost += 0.2  # Boost enemy speed
    s.fill(BLACK)
    p.draw()
    for e in es: e.draw()
    for c in cs: c.draw()
    s.blit(f.render(f"Score: {score}", 1, WHITE), (10,10))
    s.blit(f.render(f"Coins: {coins}", 1, YEL), (w-180,10))
